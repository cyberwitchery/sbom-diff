...
- make `--fail-on version-downgrade` correct for Debian and Ubuntu package versions by ordering them with the component's own ecosystem rules instead of guessing the format from the version string. a Debian revision and a semver pre-release are the same shape — `1.2.3-1ubuntu2` and `1.0.0-alpha.1` — and the two ecosystems order them in opposite directions, so a `deb` component was read as semver, where rule 11 sorts a numeric identifier below an alphanumeric one. the most common OS-package upgrade there is, `1.2.3-1ubuntu2` → `1.2.3-2`, was reported as a downgrade and exited 3, breaking CI on exactly the routine security updates the gate exists to protect; the same held for `-1build1`, `-2ubuntu0.1` and `-1+deb11u1`. components whose purl says `pkg:deb/...` are now compared with the Debian `dpkg` algorithm, so those upgrades pass and real revision, epoch and backport downgrades are still caught. because both sides then read as Debian versions, two shapes that were previously uncomparable and silently skipped by the gate are ordered as well: `1.0` → `1.0-1` (an added revision) and `1.0~rc1` → `1.0`, the standard Debian pre-release idiom, which was invisible in both directions. version-ordered pairing of same-identity components follows the same rules. every other ecosystem is unchanged: npm, cargo, Maven, Go and components with no purl keep the existing reading, so `1.0.0-alpha.1` still sorts below `1.0.0` there. `rpm` and `apk` versions still use the shape-guessing path — each needs its own algorithm rather than the Debian one
