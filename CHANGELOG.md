...
- fix invented version downgrades when the same dependency set is diffed across serialization formats: components that gain or lose their purl — and with it their ecosystem — are now paired in version order like same-ecosystem ones, so an npm SBOM re-exported as purl-less SPDX reports no version change at all instead of a spliced `1.0.0 -> 2.0.0` upgrade plus a `2.0.0 -> 1.0.0` downgrade that failed `--fail-on version-downgrade`. when a purl-less component could match several ecosystems carrying the same name, the version-nearest candidate wins rather than the alphabetically first
