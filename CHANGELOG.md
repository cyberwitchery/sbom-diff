...
- make `--fail-on version-downgrade` correct for Debian and Ubuntu package versions by ordering them with the component's own ecosystem rules instead of guessing the format from the version string. a Debian revision and a semver pre-release are the same shape — `1.2.3-1ubuntu2` and `1.0.0-alpha.1` — and the two ecosystems order them in opposite directions, so a `deb` component was read as semver, where rule 11 sorts a numeric identifier below an alphanumeric one. the most common OS-package upgrade there is, `1.2.3-1ubuntu2` → `1.2.3-2`, was reported as a downgrade and exited 3, breaking CI on exactly the routine security updates the gate exists to protect; `-1build1` and `-2ubuntu0.1` revisions behaved the same way. an inverted comparison is symmetric, so the same defect also passed genuine downgrades: `1.2.3-2` → `1.2.3-1ubuntu2` and `1.2.3-3` → `1.2.3-2ubuntu0.1` both go backwards in Debian order, and both exited 0 — for a supply-chain gate that is the direction that matters more. a `+` revision was worse still, because `+` begins semver build metadata and build metadata is excluded from precedence: `1.2.3-1+deb11u1` and `1.2.3-1+deb11u2` compared *equal*, so a Debian security point release was invisible to the gate in both directions and the `+deb11u2` → `+deb11u1` downgrade was missed outright. components whose purl says `pkg:deb/...` are now compared with the Debian `dpkg` algorithm, so the routine upgrades pass and real revision, epoch, backport and `+deb`-point-release downgrades are all caught. because both sides then read as Debian versions, two more shapes that were previously uncomparable and silently skipped by the gate are ordered as well: `1.0` → `1.0-1` (an added revision) and `1.0~rc1` → `1.0`, the standard Debian pre-release idiom, which was invisible in both directions. a leading `v`/`V` is stripped first, so `v`-prefixed Debian versions stay comparable. version-ordered pairing of same-identity components follows the same rules. every other ecosystem is unchanged: npm, cargo, Maven, Go and components with no purl keep the existing reading, so `1.0.0-alpha.1` still sorts below `1.0.0` there. `rpm` and `apk` versions still use the shape-guessing path — each needs its own algorithm rather than the Debian one
