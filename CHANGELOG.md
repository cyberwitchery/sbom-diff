...
- fix SPDX tag-value documents losing every component below an ordinary bad line: a line that is not a `key: value` pair, or one carrying a tag the parser cannot read, took the whole rest of the document with it silently, so `--fail-on added-components` waved through real additions below it and `--fail-on removed-components` reported mass removals that never happened. such lines are now skipped and named in the parser warnings instead, a value that would have crashed the run outright (an unrecognized `ExternalRef` category, checksum algorithm, relationship type, file type or annotation type) is skipped the same way rather than panicking, and a document the parser still cuts short is reported as an error naming the first package lost instead of returned as a partial SBOM. an empty or whitespace-only license expression written on the tag's own line — spec-valid input — no longer aborts the run: `PackageLicenseConcluded`, `PackageLicenseDeclared`, `LicenseConcluded`, `LicenseInfoInFile` and `SnippetLicenseConcluded` are dropped and named like any other unreadable value. an empty expression delivered as a `<text>` block still aborts, as does a non-empty expression the SPDX expression parser rejects; both do so today too
