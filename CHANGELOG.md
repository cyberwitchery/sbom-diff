...
- refactor `--fail-on` violation detection: replace `check_fail_on` (which mixed detection with `eprintln!` output and iterated `diff.changed` up to 6 times) with `collect_violations` returning structured `Violation` values from a single pass per collection; the caller now formats and prints violations, and tests can assert on specific violation types instead of just a boolean
