...
- add `Component::license_expression` and `Component::licensing` plus `Licensing`, `LicenseRequirement`, `copyleft_obligations_added` and `license_expressions_equivalent` to sbom-model, and `FieldChange::LicenseExpression` to sbom-diff; components carrying no expression are gated exactly as before, on their identifier set
