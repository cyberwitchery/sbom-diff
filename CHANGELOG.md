...
- fix ecosystem-filtered diffs (`--include-ecosystem`/`--exclude-ecosystem`) reporting an impossible `unchanged` count when a matched component migrates between ecosystems (e.g. a package moving from npm to pypi): the retained pair was counted against the old ecosystem's total but not against the new one, so it was silently absorbed into `unchanged`, which could then exceed the number of components on the new side — a self-contradictory summary. `unchanged` is now derived from the new side, consistent with the added/changed counts, so it can never exceed the new total
