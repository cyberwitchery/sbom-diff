...
- surface hash-algorithm downgrades in the text, markdown, and CSV output, not just SARIF: when a changed component's digest set drops from a strong algorithm to a weaker one (e.g. SHA-256 replaced with MD5), the text and markdown renderers now label the block `Hashes (algorithm downgrade)` and the CSV renderer uses a `hashes-downgrade` field name, matching the security-relevant signal SARIF already flagged and the `--fail-on hash-algorithm-downgrade` gate; ordinary hash changes that keep or strengthen the algorithm are still shown as a neutral `Hashes` / `hashes` block
