...
- read SPDX 2.x XML documents, the last serialization gap across the two supported standards: `--format spdx-xml` parses them explicitly and `--format auto` now detects them, so an SPDX XML file no longer falls through to the try-everything fallback and fails with a stack of unrelated parse errors naming no cause. Both `<Document>` and `<SpdxDocument>` roots are accepted, an SPDX 3.x document is rejected with the same clear unsupported-version message the JSON and tag-value paths already give, and the same document in XML and in JSON produces an identical diff. The new `SpdxReader::read_xml` mirrors the existing `read_json`/`read_tag_value` entry points
- treat the SPDX `NOASSERTION` and `NONE` supplier sentinels as "no supplier" instead of as a supplier name, matching how the parser already handles them for licenses. syft, trivy, and other generators emit `supplier: NOASSERTION` for packages whose supplier they could not determine; a diff between an SBOM that omits the field and one that writes the sentinel reported a bogus `Supplier: <none> -> NOASSERTION` change and failed `--fail-on supplier-changed` (exit 3) on that non-change. The sentinel also fed the identity hash of components that carry no purl, so the same package got a different component id depending on whether the generating tool wrote the sentinel — showing up as an unrelated add/remove pair in the diff. Real `Organization: `/`Person: ` suppliers are unaffected
- surface hash-algorithm downgrades in the text, markdown, and CSV output, not just SARIF: when a changed component's digest set drops from a strong algorithm to a weaker one (e.g. SHA-256 replaced with MD5), the text and markdown renderers now label the block `Hashes (algorithm downgrade)` and the CSV renderer uses a `hashes-downgrade` field name, matching the condition SARIF already labels and the `--fail-on hash-algorithm-downgrade` gate already acts on (the digest set's strongest algorithm got weaker); ordinary hash changes that keep or strengthen the algorithm are still shown as a neutral `Hashes` / `hashes` block
- parse the CycloneDX `metadata.component` — the BOM's primary/root component (the application, image, or container the SBOM describes, emitted by syft, trivy, and cdxgen): its name, version, purl, licenses, and hashes now appear in the diff instead of being silently dropped, so a version, license, or supplier change to the subject component is now visible. Dependency edges rooted at the primary component's bom-ref also resolve into the dependency graph instead of being discarded and reported as spurious `does not match any component` warnings. A BOM with no `metadata.component` is unaffected; a component listed in both `metadata.component` and `components[]` is de-duplicated as before
- CSV `--summary` now reports parser warnings, matching the other output formats: previously `--output csv --summary --show-warnings` silently omitted the parser warnings that the CSV full renderer and every other summary format (text, markdown, JSON) already surface, so a spreadsheet/pipeline consumer could miss that an SBOM was only partially parseable. Warnings are emitted as a leading block using the same `status,component,ecosystem,field,old_value,new_value` schema as full CSV output (`warning,old,…` / `warning,new,…` rows), so downstream CSV consumers see one consistent warning shape across full and summary output
- fix ecosystem-filtered diffs (`--include-ecosystem`/`--exclude-ecosystem`) reporting an impossible `unchanged` count when a matched component's ecosystem changes across the filter boundary (e.g. a component that keeps its name but loses its purl, so its ecosystem goes from `npm` to none): the retained pair was counted against the old ecosystem's total but not against the new one, so it was silently absorbed into `unchanged`, which could then exceed the number of components on the new side — a self-contradictory summary. `unchanged` is now derived from the new side, consistent with the added/changed counts, so it can never exceed the new total
- add three `--fail-on` CI gates for supply-chain substitution and license-compliance signals, each failing (exit 3) on a targeted change the existing gates left unguarded or buried in noise. `purl-changed` and `ecosystem-changed` fire when a component that keeps its logical identity (same name/ecosystem) has its package URL or ecosystem coordinate change between SBOM versions — a classic supply-chain substitution signal (typosquat, dependency-confusion); the diff already surfaced these coordinate changes, but unlike the sibling `license-changed`/`supplier-changed`/`version-downgrade` gates no `--fail-on` condition could act on them, and both report the old and new coordinate for each affected component. `copyleft-added` fires when a component update or a newly added component introduces a copyleft license (GPL, AGPL, LGPL, MPL, …) that was not present before — the compliance-critical direction of license change (a dependency turning copyleft) that the broader `--fail-on license-changed` gate buries in noise, since that gate fires on any license-set delta (even `MIT` → `MIT OR Apache-2.0`); copyleft is classified offline from the SPDX license list, and `LicenseRef-` and unrecognized identifiers are conservatively treated as non-copyleft
...
- fix fabricated version downgrades when an SBOM carries two versions of the same package — routine in npm and maven trees. Components that share a name and ecosystem but not a purl are now paired in version order, so `libfoo@9.0.0`/`libfoo@10.0.0` → `libfoo@9.0.1`/`libfoo@10.0.1` reports two upgrades; previously the two version lines were spliced into each other, the diff showed `Version (downgrade): 10.0.0 -> 9.0.1`, and `--fail-on version-downgrade` failed the build (exit 3) on a repo that had only upgraded. Dependency-edge reconciliation follows the corrected pairing, so the same SBOMs no longer report spurious added/removed edges either. Components whose versions cannot be ordered (opaque strings such as git hashes, a Debian/RPM version against a semver one, or a pre-release sharing a bucket with a four-part version, as `1.2.3-rc.1` does with `1.2.3.0`) keep their previous pairing rather than being force-ordered, and a package appearing only once — the overwhelming majority — is unaffected. Where one name appears both with and without an ecosystem, an exact `(ecosystem, name)` match now takes precedence over an ecosystem-less one competing for the same old component, so the more specific pairing wins
- add `Version::partial_cmp_lenient` and `versions::compare_versions` to sbom-model, exposing the lenient version ordering that `is_downgrade` already applied internally; both return `None` for pairs whose ordering is unknown (any opaque version, or a Debian/RPM version against a semver/numeric one)
